...
switch (deviceType) {
    case "DC400":
        result = Parse_Code_DC400(payloadJson, metadata);
        break;
...
//For manhole sensor with level function DC413/410
function Parse_Code_DC400(payload_in, metadata_in) {
    var height = 0;
    var temperature = 0;
    var angle = 0;
    var full_alarm = 0;
    ...
    var incomingHexData = base64ToHex(in_data.data);
    if (
        (incomingHexData.length != 34 && incomingHexData
            .length != 50) ||
        incomingHexData.substring(0, 6) != "800004" ||
    ...
    if (
        ...
        var out_h = parseInt(incomingHexData.substring(10,
            14), 16);
        var out_t = parseInt(incomingHexData.substring(16,
            18), 16);
        var out_a = parseInt(incomingHexData.substring(20,
            22), 16);
        ...
        var result = {
            ...
            telemetry: {
                ...
                temperature: out_t,
                angle: out_a,
                //alarmLevel: out_full_al == 1 ? true : false,
                alarmBattery: out_battery_al ? true :
                    false,
                alarmFall: out_fall_al ? true : false,
        ...
        var out_fw =
            parseInt(incomingHexData.substring(10, 12),
            16) +
            "." +
            parseInt(incomingHexData.substring(12, 14), 16);
        var out_upload = parseInt(incomingHexData.substring(
            14, 16), 16);
        var out_detection = parseInt(incomingHexData
            .substring(16, 18), 16);
        var out_full_th = parseInt(incomingHexData
            .substring(18, 20), 16);
        var out_fire_th = parseInt(incomingHexData
            .substring(20, 22), 16);
        var out_fall_th = parseInt(incomingHexData
            .substring(22, 24), 16);
        var out_fall_en = parseInt(incomingHexData
            .substring(24, 26), 16);
        var out_wm = parseInt(incomingHexData.substring(28,
            30), 16);
        var result = {
            deviceName: deviceName,
            deviceType: deviceType,
            attributes: {
                ...
                wireless: "LoRaWAN",
                firmware: out_fw,
                uploadInterval: out_upload,
                detectInterval: out_detection,
                //levelThreshold: out_full_th,
                fallThreshold: out_fall_th,
                fallEnable: out_fall_en,
                workMode: out_wm,
            },
        };
        return result;
