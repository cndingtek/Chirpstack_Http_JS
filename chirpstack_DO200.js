...
switch (deviceType) {
    case "DO200":
        result = Parse_Code_DO200(payloadJson, metadata);
        break;
...
//For parking sensor DO200
function Parse_Code_DO200(payload_in, metadata_in) {
    // decode payload to JSON
    var device_info = payload_in.deviceInfo;
    //var deviceType = "default";
    var incomingHexData = base64ToHex(payload_in.data);
    if (
        incomingHexData.length != 42 ||
        incomingHexData.substring(0, 6) != "800002" ||
    ...
    if (
        incomingHexData.length == 42 &&
        incomingHexData.substring(8, 10) == "15"
    ) {
        //return in_data.txInfo;
        var mag_x = parseInt(incomingHexData.substring(22,
            26), 16);
        var mag_y = parseInt(incomingHexData.substring(26,
            30), 16);
        var mag_z = parseInt(incomingHexData.substring(30,
            34), 16);
        var result = {
            ...
            telemetry: {
                ...
                zMagnet: mag_z > 32767 ? mag_z - 65536 :
                    mag_z,
                Frame_Counter: parseInt(incomingHexData
                    .substring(34, 38), 16),
            },
        };
        //console.log(result);
