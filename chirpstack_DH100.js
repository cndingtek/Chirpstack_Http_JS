...
switch (deviceType) {
    case "DH100":
        result = Parse_Code_DH100(payloadJson, metadata);
        break;
...
function Parse_Code_DH100(payload_in, metadata_in) {
    var incomingHexData = base64ToHex(payload_in.data);
    //return payload_in.data;
    //console.log(incomingHexData);
    //0x01/02 packet
    if (
        (incomingHexData.substring(0, 10) == "80000A0211" ||
            incomingHexData.substring(0, 10) == "80000A0111"
            ) &&
        ...
        var re = {
            ...
            telemetry: {
                temperature: parseInt(incomingHexData
                    .substring(16, 18), 16),
                monitorMode: parseInt(incomingHexData
                        .substring(23, 24), 16) == 0 ?
                    true : false,
                doorStatus: parseInt(incomingHexData
                        .substring(24, 25), 16) == 1 ?
                    true : false,
        ...
        incomingHexData.substring(0, 8) == "80000A03" &&
        incomingHexData.length == 24
    ) {
        //0x03 packet
        var re = {
            deviceName: deviceName,
