...
switch (deviceType) {
    case "DF200":
        result = Parse_Code_DF200(payloadJson, metadata);
        break;
...
return result;
//For dispenser level sensor DF200
function Parse_Code_DF200(payload_in, metadata_in) {
    var height = 0;
    var temperature = 0;
    var angle = 0;
    var full_alarm = 0;
    ...
    if (
        (incomingHexData.length != 30 && incomingHexData
            .length != 26) ||
    ...
    if (
        incomingHexData.length == 30 &&
        incomingHexData.substring(8, 10) == "0F"
    ) {
        var result = {
