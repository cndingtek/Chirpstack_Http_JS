...
switch (deviceType) {
    case "DC600":
        result = Parse_Code_DC600(payloadJson, metadata);
        break;
...
//For leakage sensor DC600
function Parse_Code_DC600(payload_in, metadata_in) {
    ...
    if (
        incomingHexData.length == 34 &&
        (incomingHexData.substring(6, 8) == "01" || incomingHexData.substring(6, 8) == "02")
    ) {
        var result = {
            ...
            attributes: {
                ...
                gatewayId: payload_in.rxInfo[0].gatewayId.toUpperCase(),
                gatewayLong: payload_in.rxInfo[0].location.longitude,
                gatewayLat: payload_in.rxInfo[0].location.latitude,
                band: payload_in.rxInfo[0].metadata.region_name.toUpperCase(),
                wireless: "LoRaWAN",
            },
            telemetry: {
                temperature: parseInt(incomingHexData.substring(16, 18), 16),
                level:
                    parseInt(incomingHexData.substring(24, 25), 16),
                alarmBattery:
                    parseInt(incomingHexData.substring(25, 26), 16) == 1 ? true : false,
                frameCounter: parseInt(incomingHexData.substring(26, 30), 16),
            },
        };
        ...
        var result = {
            ...
            attributes: {
                ...
                wireless: "LoRaWAN",
                firmware:
                    parseInt(incomingHexData.substring(10, 12), 16) +
                    "." +
                    parseInt(incomingHexData.substring(12, 14), 16),
                uploadInterval: parseInt(incomingHexData.substring(14, 16), 16),
                batteryThreshold: parseInt(incomingHexData.substring(22, 24), 16),
                monitorMode: parseInt(incomingHexData.substring(24, 26), 16),
            },
        };
        //console.log(result);
        return result;
