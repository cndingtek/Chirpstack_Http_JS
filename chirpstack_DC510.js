...
switch (deviceType) {
    case "DC510":
        result = Parse_Code_DC510(payloadJson, metadata);
        break;
...
function Parse_Code_DC510(payload_in, metadata_in) {

    var incomingHexData = base64ToHex(payload_in.data);
    
    //return (incomingHexData);
    //0x01/02 packet
    if (
        (incomingHexData.substring(6, 10) == "0212" ||
            incomingHexData.substring(6, 10) == "0112"
            ) 
    ) {
        var re = {
        ...
        incomingHexData.substring(6, 10) == "030D" &&
