...
switch (deviceType) {
    case "DC500":
        result = Parse_Code_DC500(payloadJson, metadata);
        break;
...
//For people counter DC510
function Parse_Code_DC500(payload_in, metadata_in) {

    var incomingHexData = base64ToHex(payload_in.data);  
    //console.log(incomingHexData);
    //0x01/02 packet
    if (
        (incomingHexData.substring(6, 10) == "0215" ||
            incomingHexData.substring(6, 10) == "0115"
        )
    ) {
        var re = {
            ...
            telemetry: {
                peopleCounter: parseInt(incomingHexData
                    .substring(10, 14), 16),
                alarmCounter: parseInt(incomingHexData
                    .substring(14, 15), 16) == 1 ?
                    true : false,
                alarmBattery: parseInt(incomingHexData
                    .substring(15, 16), 16) == 1 ?
                    true : false,
                volt: parseInt(incomingHexData
                    .substring(18, 22), 16) / 100,
                frameCounter: parseInt(incomingHexData
                    .substring(34, 38), 16),
            },
        };
    } else if (
        incomingHexData.substring(6, 10).toUpperCase() == "030D" &&
        incomingHexData.length == 26
    ) {
        //0x03 packet
        //console.log(incomingHexData);
        var re = {
            ...
            attributes: {
                ...
                uploadInterval: parseInt(incomingHexData
                    .substring(14, 16), 16),
                //detectInterval: parseInt(incomingHexData.substring(16, 18), 16),
                peopleCounterThreshold: parseInt(
                    incomingHexData.substring(16,
                        20), 16),
                batteryThreshold: parseInt(
                    incomingHexData.substring(20,
                        22), 16),
                //workMode: parseInt(incomingHexData.substring(20, 22), 16),
            },
        };
    }
    //console.log(re);
